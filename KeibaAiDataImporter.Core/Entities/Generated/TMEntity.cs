...
{
    /// <summary>
    /// JV_TM_INFO — Auto-generated entity
    /// </summary>
    public class TMEntity : JvEntity
    {
        ...
        public string TMInfo_Umaban { get; private set; } = string.Empty;
        public string TMInfo_TMScore { get; private set; } = string.Empty;
        ...
        public override void SetData(byte[] buffer)
        {
            ...
            var list_TMInfo_Umaban = new List<string>();
            for (int i = 0; i < 18; i++)
            {
                list_TMInfo_Umaban.Add(FixedLengthParser.ReadString(span, 31 + (i * 6), 2));
            }
            TMInfo_Umaban = JsonSerializer.Serialize(list_TMInfo_Umaban);
            var list_TMInfo_TMScore = new List<string>();
            for (int i = 0; i < 18; i++)
            {
                list_TMInfo_TMScore.Add(FixedLengthParser.ReadString(span, 33 + (i * 6), 4));
            }
            TMInfo_TMScore = JsonSerializer.Serialize(list_TMInfo_TMScore);
            crlf = FixedLengthParser.ReadString(span, 139, 2);
        }
    }
}
