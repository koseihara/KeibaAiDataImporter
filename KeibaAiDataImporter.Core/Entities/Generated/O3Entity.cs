...
{
    /// <summary>
    /// JV_O3_ODDS_WIDE — Auto-generated entity
    /// </summary>
    public class O3Entity : JvEntity
    {
        ...
        public string WideFlag { get; private set; } = string.Empty;
        public string OddsWideInfo_Kumi { get; private set; } = string.Empty;
        public string OddsWideInfo_OddsLow { get; private set; } = string.Empty;
        public string OddsWideInfo_OddsHigh { get; private set; } = string.Empty;
        public string OddsWideInfo_Ninki { get; private set; } = string.Empty;
        public string TotalHyosuWide { get; private set; } = string.Empty;
        ...
        public override void SetData(byte[] buffer)
        {
            ...
            WideFlag = FixedLengthParser.ReadString(span, 39, 1);
            var list_OddsWideInfo_Kumi = new List<string>();
            for (int i = 0; i < 153; i++)
            {
                list_OddsWideInfo_Kumi.Add(FixedLengthParser.ReadString(span, 40 + (i * 17), 4));
            }
            OddsWideInfo_Kumi = JsonSerializer.Serialize(list_OddsWideInfo_Kumi);
            var list_OddsWideInfo_OddsLow = new List<string>();
            for (int i = 0; i < 153; i++)
            {
                list_OddsWideInfo_OddsLow.Add(FixedLengthParser.ReadString(span, 44 + (i * 17), 5));
            }
            OddsWideInfo_OddsLow = JsonSerializer.Serialize(list_OddsWideInfo_OddsLow);
            var list_OddsWideInfo_OddsHigh = new List<string>();
            for (int i = 0; i < 153; i++)
            {
                list_OddsWideInfo_OddsHigh.Add(FixedLengthParser.ReadString(span, 49 + (i * 17), 5));
            }
            OddsWideInfo_OddsHigh = JsonSerializer.Serialize(list_OddsWideInfo_OddsHigh);
            var list_OddsWideInfo_Ninki = new List<string>();
            for (int i = 0; i < 153; i++)
            {
                list_OddsWideInfo_Ninki.Add(FixedLengthParser.ReadString(span, 54 + (i * 17), 3));
            }
            OddsWideInfo_Ninki = JsonSerializer.Serialize(list_OddsWideInfo_Ninki);
            TotalHyosuWide = FixedLengthParser.ReadString(span, 2641, 11);
            crlf = FixedLengthParser.ReadString(span, 2652, 2);
        }
    }
}
