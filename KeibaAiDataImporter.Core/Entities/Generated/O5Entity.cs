...
{
    /// <summary>
    /// JV_O5_ODDS_SANREN — Auto-generated entity
    /// </summary>
    public class O5Entity : JvEntity
    {
        ...
        public string SanrenpukuFlag { get; private set; } = string.Empty;
        public string OddsSanrenInfo_Kumi { get; private set; } = string.Empty;
        public string OddsSanrenInfo_Odds { get; private set; } = string.Empty;
        public string OddsSanrenInfo_Ninki { get; private set; } = string.Empty;
        public string TotalHyosuSanrenpuku { get; private set; } = string.Empty;
        ...
        public override void SetData(byte[] buffer)
        {
            ...
            SanrenpukuFlag = FixedLengthParser.ReadString(span, 39, 1);
            var list_OddsSanrenInfo_Kumi = new List<string>();
            for (int i = 0; i < 816; i++)
            {
                list_OddsSanrenInfo_Kumi.Add(FixedLengthParser.ReadString(span, 40 + (i * 15), 6));
            }
            OddsSanrenInfo_Kumi = JsonSerializer.Serialize(list_OddsSanrenInfo_Kumi);
            var list_OddsSanrenInfo_Odds = new List<string>();
            for (int i = 0; i < 816; i++)
            {
                list_OddsSanrenInfo_Odds.Add(FixedLengthParser.ReadString(span, 46 + (i * 15), 6));
            }
            OddsSanrenInfo_Odds = JsonSerializer.Serialize(list_OddsSanrenInfo_Odds);
            var list_OddsSanrenInfo_Ninki = new List<string>();
            for (int i = 0; i < 816; i++)
            {
                list_OddsSanrenInfo_Ninki.Add(FixedLengthParser.ReadString(span, 52 + (i * 15), 3));
            }
            OddsSanrenInfo_Ninki = JsonSerializer.Serialize(list_OddsSanrenInfo_Ninki);
            TotalHyosuSanrenpuku = FixedLengthParser.ReadString(span, 12280, 11);
            crlf = FixedLengthParser.ReadString(span, 12291, 2);
        }
    }
}
